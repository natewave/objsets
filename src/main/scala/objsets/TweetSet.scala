...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    if(that.isEmpty) this else left.union(right.union(that).incl(elem))
  }
  ...
  def mostRetweeted: Tweet = {
    ...
    val c = elem
    if( l.retweets > r.retweets) { if (l.retweets > c.retweets) l else c }
    else { if (r.retweets > c.retweets ) r else c }
  }
